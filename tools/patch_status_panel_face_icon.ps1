...
$newAllyBlock = @'
            ...
            int currentHp = unit.IsDead ? 0 : unit.CurrentHP;
'@
$text = ReplaceOptional $text $oldAllyBlock $newAllyBlock 'bind ally status FaceIcon'
...
'@
$text = InsertBeforeIfMissing $text 'private static void SetStatusFaceIcon(Transform slot, BattleUnit unit)' '        private void RedrawAllyStatusSlot(int slotNumber, BattleUnit unit)' $helper 'SetStatusFaceIcon helper'

Set-Content -Path $path -Value $text -Encoding UTF8
Write-Host 'Patched ally status panel FaceIcon binding.'
